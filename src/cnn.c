...
void conv_forward3(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
  double result_array[4] = {0.0, 0.0, 0.0, 0.0};
          ...
          for(int fy = 0; fy < 5; fy++) {
            ...
            for(int fx = 0; fx < 5; fx++) {
              ...
              if(oy >= 0 && oy < 32 && ox >=0 && ox < 32) {
                a += f->w[((5 * fy)+fx)*3+0] * V->w[((32 * oy)+ox)*3+0];
                a += f->w[((5 * fy)+fx)*3+1] * V->w[((32 * oy)+ox)*3+1];
                a += f->w[((5 * fy)+fx)*3+2] * V->w[((32 * oy)+ox)*3+2];
              }
            }
          }
          a += l->biases->w[d];
          ...
          for(int fy = 0; fy < 5; fy++) {
            ...
            for(int fx = 0; fx < 5; fx++) {
              ...
              if(oy >= 0 && oy < 16 && ox >=0 && ox < 16) {
                int fd = 0;
                __m256d unrollA = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*16+fd));
                __m256d unrollB = _mm256_loadu_pd((VW) +(((16 * oy)+ox)*16+fd));
                result0 = _mm256_mul_pd(unrollA, unrollB);

                __m256d unrollC = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*16+fd + 4));
                __m256d unrollD = _mm256_loadu_pd((VW) +(((16 * oy)+ox)*16+fd + 4));
                result = _mm256_mul_pd(unrollC, unrollD);
                sum = _mm256_add_pd(result, result0);

                __m256d unrollE = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*16+fd + 8));
                __m256d unrollF = _mm256_loadu_pd((VW) +(((16 * oy)+ox)*16+fd + 8));
                result = _mm256_mul_pd(unrollE, unrollF);
                sum = _mm256_add_pd(sum, result);

                __m256d unrollG = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*16+fd + 12));
                __m256d unrollH = _mm256_loadu_pd((VW) +(((16 * oy)+ox)*16+fd + 12));
                result = _mm256_mul_pd(unrollG, unrollH);
                sum = _mm256_add_pd(sum, result);
                _mm256_storeu_pd(result_array, sum);
                a = a+ result_array[0] + result_array[1] + result_array[2] + result_array[3];
              }
            }
          }
          a += l->biases->w[d];
          ...
          for(int fy = 0; fy < 5; fy++) {
            ...
            for(int fx = 0; fx < 5; fx++) {
              ...
              if(oy >= 0 && oy < 8 && ox >=0 && ox < 8) {
                for(int fd=0; fd < 16; fd+= 16) {
                __m256d unrollA = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*20+fd));
                __m256d unrollB = _mm256_loadu_pd((VW) +(((8 * oy)+ox)*20+fd));
                result0 = _mm256_mul_pd(unrollA, unrollB);

                __m256d unrollC = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*20+fd + 4));
                __m256d unrollD = _mm256_loadu_pd((VW) +(((8 * oy)+ox)*20+fd + 4));
                result = _mm256_mul_pd(unrollC, unrollD);
                sum = _mm256_add_pd(result, result0);

                __m256d unrollE = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*20+fd + 8));
                __m256d unrollF = _mm256_loadu_pd((VW) +(((8 * oy)+ox)*20+fd + 8));
                result = _mm256_mul_pd(unrollE, unrollF);
                sum = _mm256_add_pd(sum, result);

                __m256d unrollG = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*20+fd + 12));
                __m256d unrollH = _mm256_loadu_pd((VW) +(((8 * oy)+ox)*20+fd + 12));
                result = _mm256_mul_pd(unrollG, unrollH);
                sum = _mm256_add_pd(sum, result);
                _mm256_storeu_pd(result_array, sum);
                a = a+ result_array[0] + result_array[1] + result_array[2] + result_array[3];
              }
                for(int fd=16; fd < 20; fd+= 4) {
                  __m256d unrollI = _mm256_loadu_pd((FW) +(((5 * fy)+fx)*20+fd));
                  __m256d unrollJ = _mm256_loadu_pd((VW) +(((8 * oy)+ox)*20+fd));
                  result = _mm256_mul_pd(unrollI, unrollJ);
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  ...
  #pragma omp parallel for
    ...
    for (int j = n/4 * 4; j < n; j++) {
      ...
      copy_vol(batch[0][0], input[j]);
      net_forward(net, batch, 0, 3);
      output[j] = batch[11][0]->w[CAT_LABEL];
      free_batch(batch, 1);
    }
}
